...
class WidgetDataLoaderTest {
    ...
    // id/date are irrelevant to the loader (it never exposes them), so they are fixed here.
    private fun game(
        home: Team,
        homeScore: Int,
        visitor: Team,
    ...
    @Test
    fun `maps newest game to a score line keyed by the favorite team id`() =
        runTest {
            ...
            coEvery { getTeamGames(1) } returns
                listOf(
                    game(home = hawks, homeScore = 110, visitor = lakers, visitorScore = 104),
                    game(home = lakers, homeScore = 90, visitor = hawks, visitorScore = 88),
                )
    ...
    @Test
    fun `score line is oriented home-abbr home-score - visitor-score visitor-abbr`() =
        runTest {
            val hawks = team(1, "ATL")
            val celtics = team(3, "BOS")
            // Favorite team is the visitor here; line still reads home -> visitor.
            coEvery { getTeamGames(1) } returns
                listOf(game(home = celtics, homeScore = 99, visitor = hawks, visitorScore = 101))
    ...
    @Test
    fun `no recent game yields NoRecentGame carrying the team id from the first game-less fetch`() =
        runTest {
    ...
    @Test
    fun `null favorite team id yields Error without calling the use case`() =
        runTest {
    ...
    @Test
    fun `use case failure yields Error`() =
        runTest {
