...
- **`core:common`** — shared kernel used across layers: `error` (`DataErrorKind`, `DataException` — the cross-layer failure contract thrown by `data` and mapped to messages in `core:designsystem`), `connectivity` (`ConnectivityObserver` + Android implementation), `time` (`countdownSeconds`), and the `coreCommonModule` Koin module.
