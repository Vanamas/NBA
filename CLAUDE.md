...
- **`domain`** — pure business layer: models (`Player`, `Team`, `Game`), repository interfaces (incl. `GameRepository`), `*UseCase` classes such as `GetTeamGamesUseCase` (must live in `domain.usecase` package — Konsist rule).
