...
- **Offline-first player list** — the unfiltered list is cached in Room and served through a Paging 3 `RemoteMediator`; when the network is down an offline banner appears over the cached data instead of an error screen
...
- **Team detail** — team information (city, abbreviation, conference, division) backed by a Room read-through cache (teams are cached after the first successful load, so the detail works offline afterwards), plus the team roster and the team's recent games (last 45 days, via the `/games` endpoint). The team cache deliberately has no TTL — the 30 NBA teams are static data
- **Pull-to-refresh** — on both the player list and the team roster
...
| Persistence | [Room](https://developer.android.com/training/data-storage/room) (offline cache) |
...
- **`data`** — repository implementations, DTO→domain mappers, Paging 3 sources and the Room cache (entities, DAOs, `RemoteMediator`)
